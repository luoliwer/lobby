...
{
    NSString *imageUrl = productURL;
    
    NSRange indexRange = [productURL rangeOfString:@"jpg|"];
    if (indexRange.location == NSNotFound) {
        return imageUrl;
    }
    NSInteger index = indexRange.location + 3;
    imageUrl = [productURL substringToIndex:index];
...
- (NSDate *)dateFromString{
    if([self isEqualToString:@""]) return [NSDate date];
...
+ (NSString *)stringWithComponents:(NSArray *)substrings separateString:(NSString *)separateString {
    ...
    if (substrings && substrings.count) {
        for (int i = 0; i < substrings.count; i++) {
            if (i > 0) {
                [strM appendString:separateString];
            }
            [strM appendString:substrings[i]];
        }
    }
    return [strM copy];
}
